...
### Added
- `UnusedCustomPropertiesRule` now supports auto-fix: `--fix` deletes unused custom properties (the value entry plus every matching `propConfig` entry, including nested-children entries of object properties), for both view-level and component-level definitions. Removing a view parameter is classified unsafe — it changes the view's public interface — and only applies under `--fix-unsafe`, as does removing a property that carries an `onChange` property-change script. Backed by a new `DELETE_KEY` fix operation in the fix framework (`FixOperation`/`FixEngine`/`PathTranslator.delete_value`), which deletes dict keys only and refuses list elements to avoid index-shift hazards.
