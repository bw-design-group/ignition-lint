...
### Added
- **Lint domains** — ign-lint now lints more than Perspective views. Each kind of Ignition resource is a domain declared by a `DomainSpec` (`common/domain.py`, registry in `domains/`): a file matcher, a loader producing a `LoadedFile`, and default globs. The CLI classifies every file by type, builds one `LintEngine` per domain that has files, gives it that domain's rules from the flat config, and runs a single generic per-file path; JSON-only features (auto-fix, flattened statistics) are gated on what the loader returned. Adding a domain is a pure addition with no edits to `cli.py`/`linter.py`, proven by a test that registers a throwaway domain at runtime. [858c527]
- **`scripting` domain** — Ignition project-library modules (`ignition/script-python/**/code.py`, or a `code.py` with a sibling `resource.json`) are loaded as first-class `ScriptModule` nodes (dotted path, source, `resource.json` metadata) plus one `ScriptPackage` node per enclosing folder. New node types `script_module` and `script_package` are deliberately not part of `ALL_SCRIPTS`. Library files are opt-in: pass them explicitly, via `--files` globs, or through the new `ign-lint-scripting` pre-commit hook. [858c527]
- **`LibraryScriptPylintRule`** — runs pylint directly on library modules with configuration independent from the Perspective rule: its own `pylintrc` (default `.config/.ignition-library-pylintrc`, walking up from the working directory, then a new bundled copy), its own `category_mapping`, and `additional_builtins`. Top-level library packages beside the module are discovered from the `script-python` folder and declared as builtins automatically. Real line numbers, category-grouped output. [00ee54f]
- **`LibraryNamePatternRule`** — applies `NamePatternRule`'s conventions to library packages (`script_package`) and modules (`script_module`); snake_case by default, per-type overrides via `node_type_specific_rules`, each package reported once per run. No auto-fix. [803283f]
- Configuration stays a flat map of rule name → settings; rule names are unique, so each entry is routed to its domain automatically and existing configs work unchanged. The default `.ignition-lint-precommit.json` referenced by the shipped pre-commit hooks is bundled inside the package (`src/ignition_lint/.config/`); when a consumer repository has no file of that name, `load_config` falls back to the bundled copy and prints which file it used, so both hooks work out of the box. A root-level file of the same name overrides it. [899649d]
- Test fixtures are split per domain: `tests/cases/views/` (Perspective) and `tests/cases/scripting/` (library packages `clean`, `violations`, `badModule_name`), each bind-mounted into the docker Designer so resources authored there land in the repo. [5cfc565]
- `view` node type — the object model now exposes the view itself, with `name` (the directory containing `view.json`), `folder_path` (every directory between the views root and the view directory) and `view_path` (`Folder/Sub/Name`). Derived from the file location rather than the JSON contents: the views root is `com.inductiveautomation.perspective/views`, falling back to the bare `views` directory nearest the file; with no root the view is still named after its directory but no folders are reported, and a `view.json` directly inside the views root yields no node. The node also summarizes the view: `root_container_type` (the root component's type, e.g. `ia.container.flex`), `default_size` (the view's `props.defaultSize` as a `width`/`height` dict, each `None` when not declared; no platform default is assumed) and `node_counts` / `total_nodes` over every other modeled node. `LintEngine.process` already received the source path, so the node appears wherever a path is supplied (CLI, `--stats-only`, `--debug-nodes view`, `--analyze-rules`); callers that build a model without a path get an empty `view` collection. [ee00394]
- `NamePatternRule` can target `"view"` to enforce a naming convention on the view name and every parent folder below the views root, e.g. PascalCase views and folders instead of Title Case. Folder violations are labelled `Folder name` and share the `view` configuration, including a `"view"` entry in `node_type_specific_rules` (use a `pattern` there to allow more than one style, `"check_view_folders": false` to validate the view name only, or `skip_names` to exempt individual folders). The implicit `root` skip applies to the root component and property only, never to a view or folder. Opt-in: existing configs targeting components are unaffected, and leaving `view` untargeted skips the checks. No auto-fix is offered for view or folder renames. `rule_config.json` gains a `view` example. [53e6083]
- `tests/cases/views/Naming/` fixtures with nested view folders (plain directories, no resource files) exercising the new node type. The test harness (`BaseRuleTest`, config framework) now passes the view file path to `LintEngine.process`, matching the CLI. [d66ac16]
- Documentation for the `view` node (api-reference, architecture, configuration node-type list, `CLAUDE.md`) and for view/folder naming in the `NamePatternRule` guide and full reference. [5ced689]
- Merge integration (view metadata + lint domains): the Perspective domain loader passes the file path into `ViewModelBuilder.build_model`, and `collect_nodes` iterates `NODE_COLLECTIONS` (which includes `view`), so the `view` node is built and visited on the generic `process_file` CLI path, not only via `LintEngine.process`. `LintEngine.get_model_statistics` / `debug_nodes` / `analyze_rule_impact` accept a `LoadedFile`, a flattened dict (+ optional `source_file_path`) or nothing (last loaded file). The view-naming fixtures live under `tests/cases/views/Naming/<Folder>/<View>/view.json` (no inner `views/` segment: `tests/cases/views` is the views root for every fixture), golden tooling is keyed relative to `tests/cases/views` and builds models with the real repo-relative path, and all golden files are regenerated.

### Changed
- **`PylintScriptRule` renamed to `PerspectiveScriptPylintRule`** so the name says what it lints. The old name remains a Python alias and a config/`--fix-rules` alias that prints one deprecation line per run; it is not registered as a second rule, so nothing runs twice. Fixes it emits now carry `rule_name="PerspectiveScriptPylintRule"`, which also fixes `--fix-rules PerspectiveScriptPylintRule` filtering out its own fixes. Shared pylint plumbing (rcfile resolution, in-process run, output parsing, category grouping) moved to `rules/scripts/pylint_support.py`; pylint is always run with `--jobs=1`. [ecb8012]
- `LintEngine` is domain-agnostic: `process_file(path, spec)` is the generic entry point; `process(flattened_json, ...)` stays as the Perspective-only path used by tests and `scripts/generate_debug_files.py`. Statistics, `--debug-nodes`, `--analyze-rules` and `--debug-output` read the last loaded file. [899649d]
- `--debug-output` now writes one folder per linted file, mirroring the source path (`<dir>/views/Dashboard/{flattened,model,stats}.json`, the golden-file layout) instead of `<stem>_*.json` files that every `view.json` overwrote. The directory carries an `.ignition-lint-debug` marker that also serves as a manifest of the folders ign-lint wrote; at the start of each run exactly those listed folders are removed (leaves written in the last five seconds are kept for parallel pre-commit batches), and nothing else in the directory is ever touched. The marker is only created in a directory ign-lint created itself or found empty; a pre-existing directory with other files is written into with a warning and never cleaned. `flattened.json` is skipped for non-JSON domains. [899649d]
- `collect_files` returns files grouped by domain; explicit paths no domain recognises are reported and skipped, and glob results are filtered through the domain registry instead of by the literal name `view.json`. [899649d]
- Bundled Perspective pylintrc disables `use-maxsplit-arg` (Jython 2.7 only takes `maxsplit` positionally, so the suggested keyword form is a `TypeError`) and `import-outside-toplevel` (an artifact of aggregating every script of a view into one file). The library pylintrc disables `use-maxsplit-arg` for the same reason. [bc79e66]
- Both pylint rules survive an rcfile pylint cannot start with (e.g. `jobs=abc`): the `SystemExit` pylint raises despite `exit=False` is caught in `run_pylint` and reported as one fatal `F0001` violation quoting pylint's message, instead of killing the CLI with exit 32 and no output. pylint's own configuration diagnostics (`E0015 unrecognized-option`, attributed to the rcfile) are reported without a line number and name the rcfile rather than being pinned to line 1 of the module.
- `LibraryScriptPylintRule` reads the rcfile's `additional-builtins` through pylint's own configuration loader (INI or TOML, inline comments, multi-line values) instead of a hand-rolled `RawConfigParser` pass that produced garbage names and, because a command-line value replaces the rcfile's, made `system` an undefined variable.
- Both bundled rcfiles declare the Jython 2.7 builtins (`xrange`, `basestring`, `raw_input`, `reduce`, `cmp`, `unichr`, `file`, `execfile`, `reload`, `apply`, `buffer`, `intern`, `coerce`, `StandardError`) so valid Jython modules no longer produce undefined-variable errors.
- A `code.py` sitting directly inside `script-python/` is rejected with a clear message instead of being modelled as a module named `script-python`.
- Files of a domain whose rules are all disabled are now listed as skipped per file and counted in the summary (`Files skipped (no rules for their domain)`) instead of being dropped from the run silently. `--fix-rules PylintScriptRule` prints the same deprecation line as the config alias. `--debug-output` pointing at an existing file exits with a clear error instead of a traceback.
- CI's "Test CLI functionality" step lints `tests/cases/views/PascalCase/view.json` (the fixture moved); it previously matched nothing and passed vacuously.
- Pre-commit docs: the recommended `exclude` is `'^tests/'`; the former `'.*test.*\.json$'` alternative silently excluded any view whose path contains `test` (e.g. `LatestStatus`). Hook ids in the CLAUDE.md examples corrected to `ign-lint`.
- The repo's `pylint-quick` pre-commit hook runs inside the poetry environment so `ignition_lint` resolves to `src/`, and skips the Jython fixture scripts under `tests/cases/`. [5cfc565]
- Golden-file tooling (`scripts/generate_debug_files.py`, `tests/unit/test_golden_files.py`) discovers `view.json` at any depth under `tests/cases`, keyed by the path relative to `tests/cases`, so nested fixtures get their own golden directories; models are built with a repo-relative source path so the `view` node is machine-independent. All golden files regenerated to include the `view` collection. [3a67700]

### Documentation
- Guide and reference pages for `LibraryScriptPylintRule` and `LibraryNamePatternRule`; Perspective pylint pages retitled with the alias note; configuration, CLI, pre-commit (separate hooks, migrating from a raw `pylint` hook), debug-output and architecture (lint domains, "Adding a domain" checklist) pages, README, CLAUDE.md and the intro rules table updated. [f20efd3]
