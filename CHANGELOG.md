...
- `tests/cases/ViewNaming/views/` fixtures with nested view folders (plain directories, no resource files) exercising the new node type. The test harness (`BaseRuleTest`, config framework) now passes the view file path to `LintEngine.process`, matching the CLI. [d66ac16]
