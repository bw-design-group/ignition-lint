...
- Configuration stays a flat map of rule name → settings; rule names are unique, so each entry is routed to its domain automatically and existing configs work unchanged. The previously missing `.ignition-lint-precommit.json` referenced by the pre-commit hooks is added. [899649d]
...
- `--debug-output` now writes one folder per linted file, mirroring the source path (`<dir>/views/Dashboard/{flattened,model,stats}.json`, the golden-file layout) instead of `<stem>_*.json` files that every `view.json` overwrote. The directory carries an `.ignition-lint-debug` marker; at the start of each run entries older than five seconds are removed from marked directories only, so stale output does not accumulate across a thousand views and parallel pre-commit batches do not erase each other. `flattened.json` is skipped for non-JSON domains. [899649d]
