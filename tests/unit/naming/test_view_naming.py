...
Test views live under tests/cases/ViewNaming/views/, where views/ stands in for the
Perspective views root and folders are plain directories.
"""
...
PASCAL_VIEW = "ViewNaming/views/PascalFolder/PascalView"
TITLE_VIEW = "ViewNaming/views/Title Case Folder/Title Case View"
DEEP_VIEW = "ViewNaming/views/PascalFolder/Nested Sub Folder/DeepView"
...
class TestViewNamingPascalCase(BaseRuleTest):
	...
	def test_title_case_folder_and_view_both_fail(self):
		...
		self.assertIn(
			"Title Case Folder/Title Case View: Name 'Title Case View' doesn't follow PascalCase for view "
			"(suggestion: 'TitleCaseView')", errors
		)
		self.assertIn(
			"Title Case Folder/Title Case View: Folder name 'Title Case Folder' doesn't follow PascalCase "
...
class TestViewNamingUnanchoredPaths(BaseRuleTest):
	"""Without a views root the view name is still checked; no folders are inferred."""
	rule_config: Dict[str, Dict[str, Any]]  # Override base class to make non-optional
	...
	def test_flat_case_directory_name_is_validated(self):
		"""tests/cases/Title Case/view.json is named after its directory and fails PascalCase."""
