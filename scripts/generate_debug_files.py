...
This script processes all Perspective test cases (any depth under tests/cases/views/) and generates
debug files (flattened JSON, model state, statistics) in tests/debug/cases/{case_key}/ to avoid
...
Usage:
  ...
  python scripts/generate_debug_files.py "Naming/PascalFolder/PascalView"  # Nested case (path under tests/cases/views)
...
# Perspective fixtures only; tests/cases/scripting holds the scripting-domain fixtures.
CASES_DIR = REPO_ROOT / 'tests' / 'cases' / 'views'


def case_key(case_dir: Path) -> str:
	"""
	Identify a test case by its path relative to tests/cases/views, e.g. 'PascalCase' or
	'Naming/Title Case Folder/Title Case View'. Used for the debug directory,
...
def golden_source_path(case_dir: Path) -> str:
	"""
	Repo-relative path of the fixture's view.json (``tests/cases/views/<key>/view.json``).

	Handing the builder this path instead of an absolute one keeps the View node
	(name, folder path, ``views_root_found``) identical on every machine. ``tests/cases/views``
	is the first bare ``views`` segment, so it acts as the views root for every fixture.
	Must match tests/unit/test_golden_files.py.
	"""
	return (CASES_DIR.relative_to(REPO_ROOT) / case_key(case_dir) / 'view.json').as_posix()


def get_test_cases() -> List[Path]:
	"""Get all test case directories (at any depth under tests/cases/views) that contain a view.json."""
...
def generate_debug_files_for_case(case_dir: Path, lint_engine: LintEngine) -> bool:
	...
	try:
		...
		# Build the model with the repo-relative source path (see golden_source_path).
		source_path = golden_source_path(case_dir)
		lint_engine.flattened_json = flattened_json
		lint_engine.view_model = lint_engine.get_view_model(source_path)
		...
		# Save statistics
		stats = lint_engine.get_model_statistics(flattened_json, source_path)
...
This directory contains debug information generated from `tests/cases/views/{key}/view.json`:
